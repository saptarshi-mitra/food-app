...
export class AuthService {
  ...
  appKey = "AIzaSyAHwgKUz04OEuff0r1e5PcJN1T94BbKsR8";
  ...
  signIn(email: string, password: string) {
    return this.http.post<AuthResponseData>(
      `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${this.appKey}`,
      {
        email: email,
        password: password,
  ...
  signUp(email: string, password: string) {
    return this.http.post<AuthResponseData>(
      `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${this.appKey}`,
      {
        email: email,
        password: password,
        returnSecureToken: true
  ...
  //add username to newly registered user
